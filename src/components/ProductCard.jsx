  ...
  return (
    ...
    >
      <div style={{background:'var(--grey)',aspectRatio:1,display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}>
        ...
        <button onClick={e=>{e.stopPropagation();setWished(v=>!v)}} style={{position:'absolute',top:12,right:12,width:32,height:32,background:'white',border:'none',borderRadius:'50%',fontSize:14,cursor:'pointer',boxShadow:'0 2px 8px rgba(0,0,0,0.1)',color:wished?'var(--accent)':'inherit'}}>
          {wished?'♥':'♡'}
        </button>
      </div>
