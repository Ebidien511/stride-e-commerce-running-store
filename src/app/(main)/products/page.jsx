...
function ProductsContent() {
  ...
  const aiRankedIds = aiParam ? JSON.parse(decodeURIComponent(aiParam)) : null
  const router = useRouter()
  console.log('AI param raw:', aiParam)
  console.log('AI ranked IDs:', aiParam ? JSON.parse(decodeURIComponent(aiParam)) : null)
  ...
  const filtered = useMemo(() => {
    ...
    if (aiRankedIds) {
      const ranked = aiRankedIds.map(id => results.find(p => p.id === id)).filter(Boolean)
      const unranked = results.filter(p => !aiRankedIds.includes(p.id))
      return [...ranked, ...unranked]
    }

    return results.sort((a, b) => sort === 'price-asc' ? a.price - b.price : sort === 'price-desc' ? b.price - a.price : 0)
  }, [products, activePill, checkedArch, checkedBrand, priceMin, priceMax, sort, q, aiRankedIds])
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 32, padding: '32px 48px 80px', alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
            {aiPromptParam && (
              <div style={{ gridColumn: '1/-1', background: '#fff0ed', border: '1px solid rgba(255,77,28,0.2)', borderRadius: 12, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontSize: 20 }}>🤖</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>AI Results for: <em>"{decodeURIComponent(aiPromptParam)}"</em></div>
                  <div style={{ fontSize: 11, color: 'var(--mid)', marginTop: 2 }}>Products ranked by best match to your request</div>
                </div>
                <button onClick={() => router.push('/products')} style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 600, background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}>Clear ✕</button>
              </div>
            )}
            {filtered.length === 0
