...
function SaveBtn({ onClick, saved }) {
  return (
    <button onClick={onClick} style={{padding:'10px 24px',background:'var(--black)',color:'white',border:'none',borderRadius:8,fontSize:13,fontWeight:700,cursor:'pointer',transition:'all 0.2s'}}
...
export default function ProfilePage() {
  ...
  // Personal info
  const [info, setInfo] = useState({ firstName:'Alex', lastName:'Runner', email:'alex@example.com', phone:'+27 82 000 0000' })
  const [infoErr, setInfoErr] = useState({})
  const [infoSaved, setInfoSaved] = useState(false)

  // Address
  const [addr, setAddr] = useState({ street:'14 Long Street', city:'Cape Town', province:'Western Cape', postal:'8001' })
  ...
  const saveInfo = () => {
    ...
    setInfoErr(e)
    if (!Object.keys(e).length) { setInfoSaved(true); setTimeout(()=>setInfoSaved(false), 3000) }
  }
  ...
  const saveAddr = () => {
    ...
    setAddrErr(e)
    if (!Object.keys(e).length) { setAddrSaved(true); setTimeout(()=>setAddrSaved(false), 3000) }
  }
  ...
  const savePw = () => {
    ...
    setPwErr(e)
    if (!Object.keys(e).length) { setPwSaved(true); setPw({current:'',newPw:'',confirm:''}); setPwStrength(0); setTimeout(()=>setPwSaved(false), 3000) }
  }
    ...
    <div style={{padding:'40px 48px 80px',maxWidth:1200,margin:'var(--nav-h) auto 0'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:24,marginBottom:48,padding:'28px 32px',background:'var(--black)',borderRadius:20,color:'white'}}>
        ...
        <div>
          ...
          <p style={{color:'rgba(255,255,255,0.5)',fontSize:13}}>{info.email} · Member since Jan 2025</p>
        </div>
      </div>
            ...
            <>
              ...
              <div style={{background:'white',border:'1px solid var(--border)',borderRadius:16,padding:28}}>
                ...
                <div style={{marginTop:8}}>
                  <SaveBtn onClick={()=>{setProfileSaved(true);setTimeout(()=>setProfileSaved(false),3000)}} saved={profileSaved}/>
                </div>
              </div>
            </>
          )}
